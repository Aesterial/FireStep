...
func _succeed() -> void:
	phase = Phase.SUCCESS
	set_process(false)
	banner_changed.emit("Этап в цехе завершён. Переходите к эвакуации.", "safe")
	...
	result_requested.emit(
		...
		"Оборудование остановлено, безопасная дистанция создана, помощь вызвана. Осталось завершить эвакуацию."
	)
