...
func _ready() -> void:
	...
	GameSession.input_mode_changed.connect(_on_input_mode_changed)


func _on_input_mode_changed(_mode: String) -> void:
	if current_interactable != null:
		prompt_changed.emit(GameSession.get_prompt_string(current_interactable.prompt_text))


func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and gameplay_enabled and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		rotate_y(-event.relative.x * GameSession.mouse_sensitivity)
		head.rotate_x(-event.relative.y * GameSession.mouse_sensitivity)
	...
	if event.is_action_pressed("toggle_pointer"):
	...
	if gameplay_enabled and event.is_action_pressed("interact") and current_interactable != null:
...
func _physics_process(delta: float) -> void:
	...
	if gameplay_enabled:
		move_vector = Input.get_vector("move_left", "move_right", "move_back", "move_forward")
		
		if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
			var look_vec = Input.get_vector("look_left", "look_right", "look_up", "look_down")
			if look_vec.length() > GameSession.gamepad_deadzone:
				rotate_y(-look_vec.x * GameSession.gamepad_look_sensitivity * delta)
				head.rotate_x(-look_vec.y * GameSession.gamepad_look_sensitivity * delta)
				head.rotation.x = clamp(head.rotation.x, -LOOK_LIMIT, LOOK_LIMIT)

	var speed: float = WALK_SPEED
	var sprinting: bool = gameplay_enabled and Input.is_action_pressed("sprint")
	if sprinting:
		speed = SPRINT_SPEED
